...
function App() {
  return (
    <div className="App">
      ...
      {/* Nested routing, but the problem in this routing is that the parent element doesnot let the child elements load*/}
      <Routes>
        {/* parent route is / or home */}
        <Route path="/" element={<div><Outlet />Home</div>}>
        {/* child routes must go through home routes means for /about first / then about */}
          <Route path="/about" element={<div>about</div>} />
          <Route path="/contactus" element={<div>contact us</div>} />
          {/* * is used to handle any other than above specified route paths */}
          <Route path="*" element={<div>404 NOt found</div>} />
        </Route> {/* parent route closing tag */}
      </Routes>

      {/* So to prevent problem in nested routing, outlet tag is used, added the <Outlet /> tag in the parent tag at line 34 as home is the parent route */}
      ...
      {/* So to solve the problem at line 45 just do some steps: 1. Create one componenet name it as you like 2. Transfer the outlet tag to the newly formed component 3. Make that new componenet as a parent of the routes 4. Now add a new rout with element as home and put the index keyword inside that route */}
